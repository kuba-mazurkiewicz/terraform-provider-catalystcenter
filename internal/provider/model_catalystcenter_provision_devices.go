...
func (data *ProvisionDevices) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.ProvisionDevices {
		keys := [...]string{"id", "siteId", "networkDeviceId", ""}
		keyValues := [...]string{data.ProvisionDevices[i].Id.ValueString(), data.ProvisionDevices[i].SiteId.ValueString(), data.ProvisionDevices[i].NetworkDeviceId.ValueString(), strconv.FormatBool(data.ProvisionDevices[i].Reprovision.ValueBool())}
		...
		if value := r.Get(""); value.Exists() && !data.ProvisionDevices[i].Reprovision.IsNull() {
			...
			data.ProvisionDevices[i].Reprovision = types.BoolNull()
		}
...
func (data *ProvisionDevices) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
	...
	res = res.Get("response")
	for i := range data.ProvisionDevices {
		keys := [...]string{"siteId", "networkDeviceId", ""}
		keyValues := [...]string{data.ProvisionDevices[i].SiteId.ValueString(), data.ProvisionDevices[i].NetworkDeviceId.ValueString(), strconv.FormatBool(data.ProvisionDevices[i].Reprovision.ValueBool())}
