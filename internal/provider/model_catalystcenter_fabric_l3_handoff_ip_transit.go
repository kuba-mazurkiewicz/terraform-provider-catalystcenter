...
type FabricL3HandoffIPTransitL3HandoffIpTransits struct {
	Id                             types.String `tfsdk:"id"`
...
func (data FabricL3HandoffIPTransit) toBody(ctx context.Context, state FabricL3HandoffIPTransit) string {
	...
	if len(data.L3HandoffIpTransits) > 0 {
		...
		for _, item := range data.L3HandoffIpTransits {
			itemBody := ""
			if !item.Id.IsNull() {
...
func (data *FabricL3HandoffIPTransit) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res; value.Exists() && len(value.Array()) > 0 {
		data.L3HandoffIpTransits = make([]FabricL3HandoffIPTransitL3HandoffIpTransits, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := FabricL3HandoffIPTransitL3HandoffIpTransits{}
			...
			data.L3HandoffIpTransits = append(data.L3HandoffIpTransits, item)
			return true
		})
	}
}
...
func (data *FabricL3HandoffIPTransit) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	res = res.Get("response")
	for i := range data.L3HandoffIpTransits {
		keys := [...]string{"vlanId"}
		keyValues := [...]string{strconv.FormatInt(data.L3HandoffIpTransits[i].VlanId.ValueInt64(), 10)}
		...
		if value := r.Get("id"); value.Exists() && !data.L3HandoffIpTransits[i].Id.IsNull() {
			data.L3HandoffIpTransits[i].Id = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].Id = types.StringNull()
		}
		if value := r.Get("fabricId"); value.Exists() && !data.L3HandoffIpTransits[i].FabricId.IsNull() {
			data.L3HandoffIpTransits[i].FabricId = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].FabricId = types.StringNull()
		}
		if value := r.Get("transitNetworkId"); value.Exists() && !data.L3HandoffIpTransits[i].TransitNetworkId.IsNull() {
			data.L3HandoffIpTransits[i].TransitNetworkId = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].TransitNetworkId = types.StringNull()
		}
		if value := r.Get("networkDeviceId"); value.Exists() && !data.L3HandoffIpTransits[i].NetworkDeviceId.IsNull() {
			data.L3HandoffIpTransits[i].NetworkDeviceId = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].NetworkDeviceId = types.StringNull()
		}
		if value := r.Get("interfaceName"); value.Exists() && !data.L3HandoffIpTransits[i].InterfaceName.IsNull() {
			data.L3HandoffIpTransits[i].InterfaceName = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].InterfaceName = types.StringNull()
		}
		if value := r.Get("externalConnectivityIpPoolName"); value.Exists() && !data.L3HandoffIpTransits[i].ExternalConnectivityIpPoolName.IsNull() {
			data.L3HandoffIpTransits[i].ExternalConnectivityIpPoolName = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].ExternalConnectivityIpPoolName = types.StringNull()
		}
		if value := r.Get("virtualNetworkName"); value.Exists() && !data.L3HandoffIpTransits[i].VirtualNetworkName.IsNull() {
			data.L3HandoffIpTransits[i].VirtualNetworkName = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].VirtualNetworkName = types.StringNull()
		}
		if value := r.Get("vlanId"); value.Exists() && !data.L3HandoffIpTransits[i].VlanId.IsNull() {
			data.L3HandoffIpTransits[i].VlanId = types.Int64Value(value.Int())
		} else {
			data.L3HandoffIpTransits[i].VlanId = types.Int64Null()
		}
		if value := r.Get("tcpMssAdjustment"); value.Exists() && !data.L3HandoffIpTransits[i].TcpMssAdjustment.IsNull() {
			data.L3HandoffIpTransits[i].TcpMssAdjustment = types.Int64Value(value.Int())
		} else {
			data.L3HandoffIpTransits[i].TcpMssAdjustment = types.Int64Null()
		}
		if value := r.Get("localIpAddress"); value.Exists() && !data.L3HandoffIpTransits[i].LocalIpAddress.IsNull() {
			data.L3HandoffIpTransits[i].LocalIpAddress = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].LocalIpAddress = types.StringNull()
		}
		if value := r.Get("remoteIpAddress"); value.Exists() && !data.L3HandoffIpTransits[i].RemoteIpAddress.IsNull() {
			data.L3HandoffIpTransits[i].RemoteIpAddress = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].RemoteIpAddress = types.StringNull()
		}
		if value := r.Get("localIpv6Address"); value.Exists() && !data.L3HandoffIpTransits[i].LocalIpv6Address.IsNull() {
			data.L3HandoffIpTransits[i].LocalIpv6Address = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].LocalIpv6Address = types.StringNull()
		}
		if value := r.Get("remoteIpv6Address"); value.Exists() && !data.L3HandoffIpTransits[i].RemoteIpv6Address.IsNull() {
			data.L3HandoffIpTransits[i].RemoteIpv6Address = types.StringValue(value.String())
		} else {
			data.L3HandoffIpTransits[i].RemoteIpv6Address = types.StringNull()
		}
	}
}
...
func (data *FabricL3HandoffIPTransit) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
	...
	for i := range data.L3HandoffIpTransits {
		...
		if data.L3HandoffIpTransits[i].Id.IsUnknown() {
