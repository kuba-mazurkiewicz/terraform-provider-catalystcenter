...
type ControlPlaneDevice struct {
...
func (data ControlPlaneDevice) getPath() string {
...
func (data ControlPlaneDevice) toBody(ctx context.Context, state ControlPlaneDevice) string {
	body := ""
	put := false
...
func (data *ControlPlaneDevice) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("routeDistributionProtocol"); value.Exists() {
		...
		data.RouteDistributionProtocol = types.StringValue("LISP_BGP")
	}
}
...
func (data *ControlPlaneDevice) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("routeDistributionProtocol"); value.Exists() && !data.RouteDistributionProtocol.IsNull() {
		data.RouteDistributionProtocol = types.StringValue(value.String())
	} else if data.RouteDistributionProtocol.ValueString() != "LISP_BGP" {
...
func (data *ControlPlaneDevice) isNull(ctx context.Context, res gjson.Result) bool {
	if !data.SiteNameHierarchy.IsNull() {
		return false
	}
	if !data.RouteDistributionProtocol.IsNull() {
